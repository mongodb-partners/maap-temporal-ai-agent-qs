...
class TransactionActivities:
    ...
    @activity.defn
    async def validate_and_hold_funds(self, transaction_details: TransactionDetails) -> Dict[str, Any]:
    ...
    @activity.defn
    async def enrich_transaction_data(self, transaction_details: TransactionDetails) -> Dict[str, Any]:
        ...
        try:
            ...
            # Combine risk flags
            risk_flags = transaction_details.risk_flags.copy() if transaction_details.risk_flags else []
            ...
            if customer_history.get("common_recipients"):
                if transaction_details.recipient.get("name") not in customer_history["common_recipients"]:
            ...
            enriched_data = {
                ...
                "enrichment_time_ms": int((datetime.now(timezone.utc) - start_time).total_seconds() * 1000)
            }
    ...
    def _calculate_velocity_metrics(self, customer_id: str, sender_account: str) -> Dict[str, Any]:
        ...
        try:
            ...
            transactions_1h = list(db_sync[config.TRANSACTIONS_COLLECTION].find({
                "$or": [
            ...
            transactions_24h = list(db_sync[config.TRANSACTIONS_COLLECTION].find({
                "$or": [
                    {"sender.customer_id": customer_id},
                    {"sender.account_number": sender_account}
                ],
            ...
            if transactions_1h:
                last_transaction_time = max(t.get("created_at") for t in transactions_1h if t.get("created_at"))
    ...
    @activity.defn
    async def perform_risk_assessment(self, enriched_data: Dict[str, Any]) -> RiskAssessment:
        ...
        try:
            ...
            if not kyc_status:
                kyc_status = enriched_data["customer_history"].get("kyc_status", "pending")
            ...
            if transaction["transaction_type"] == "international":
                compliance_checks["ofac_check"] = transaction["recipient"].get("country") not in config.HIGH_RISK_COUNTRIES
                compliance_checks["fatf_check"] = True
            
            # Determine if enhanced due diligence is required
            requires_edd = (
                base_risk > 70 or
                transaction["amount"] > 100000 or
                "high_risk_country" in enriched_data.get("risk_flags", []) or
                rule_results.get("recommended_action") in ["escalate", "reject"]
            )
            
            assessment = RiskAssessment(
                risk_score=base_risk,
                risk_level=RiskEngine.determine_risk_level(base_risk).value,
                risk_factors=risk_analysis.get("key_risk_factors", []) + enriched_data.get("risk_flags", []),
            ...
            try:
                metric = SystemMetric(
                    ...
                    value=int((datetime.now(timezone.utc) - start_time).total_seconds() * 1000),
                    unit="milliseconds",
                    dimensions={
    ...
    @activity.defn
    async def find_similar_transactions(self, enriched_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            similar_cases = await DecisionRepository.hybrid_search_similar_transactions(
                embedding,
                transaction,
                limit=config.MAX_SIMILAR_CASES
            )
    ...
    async def ai_decision_analysis(
        ...
        try:
            ...
            compliance_checks = risk_assessment.compliance_checks if hasattr(risk_assessment, 'compliance_checks') else {}
            if compliance_checks and not all(compliance_checks.values()):
                ...
                critical_failures = [check for check in failed_checks if check in ["ofac_check", "sanctions_check"]]
                if critical_failures:
                    # Return rejection decision instead of raising error
                    activity.logger.error(f"Critical compliance checks failed: {', '.join(critical_failures)}")
                    return {
                        "decision": "reject",
                        "confidence": 100.0,
                        "reasoning": f"Transaction rejected due to compliance violation: {', '.join(critical_failures)}",
                        "risk_factors": ["compliance_violation", "sanctions_risk"] + enriched_data.get("risk_flags", []),
                ...
                if failed_checks:
                    logger.warning(f"Non-critical compliance checks failed: {', '.join([c for c in failed_checks if c not in critical_failures])}")
            ...
            if ai_result.get("confidence", 0) < 70 and rule_results.get("recommended_action"):
                ...
                mapped_action = action_mapping.get(rule_results["recommended_action"], "escalate")
                ai_result["decision"] = mapped_action
                ai_result["reasoning"] += f" | Overridden by rule engine: {rule_results['recommended_action']} (mapped to {mapped_action})"
            
            # Enhance AI result
            ai_result["risk_assessment"] = {
                "risk_score": risk_assessment.risk_score if hasattr(risk_assessment, 'risk_score') else 50,
                "risk_level": risk_assessment.risk_level if hasattr(risk_assessment, 'risk_level') else "medium",
                "risk_factors": risk_assessment.risk_factors if hasattr(risk_assessment, 'risk_factors') else []
            }
            ai_result["similar_case_ids"] = [
                {"transaction_id": case.get("transaction_id"), "similarity": case.get("similarity_score", 0)}
                for case in similar_cases[:3]
            ]
            ai_result["processing_time_ms"] = int((datetime.now(timezone.utc) - start_time).total_seconds() * 1000)
    ...
    async def store_decision(
        ...
        try:
            ...
            decision = TransactionDecision(
                ...
                model_version=config.BEDROCK_MODEL_VERSION,
            ...
            try:
                audit_event = AuditEvent(
                    ...
                    event_data={
                        ...
                        "risk_score": ai_result.get("risk_assessment", {}).get("risk_score", 50),
                        "rules_triggered": ai_result.get("rules_triggered", []),
                        "similar_cases_used": len(ai_result.get("similar_case_ids", []))
                    }
                )
            ...
            try:
                TransactionRepository.update_status_sync(
                    transaction_id,
                    status_map.get(ai_result["decision"], "processing")
                )
            except Exception as status_error:
                activity.logger.warning(f"Failed to update transaction status: {status_error}")
            ...
            for rule_id in ai_result.get("rules_triggered", []):
                try:
                    ...
                    activity.logger.warning(f"Failed to update rule metrics: {rule_error}")
            
            activity.logger.info(f"Stored decision {decision_id} for transaction {transaction_id}")
    ...
    async def queue_for_human_review(
        ...
        try:
            ...
            review = HumanReview(
                transaction_id=transaction_id,
                priority=priority,
                sla_deadline=datetime.now(timezone.utc) + timedelta(hours=4 if priority == "urgent" else 24),
                ai_recommendation={
            ...
            try:
                audit_event = AuditEvent(
                    ...
                    transaction_id=transaction_id,
                    event_data={
                        "reason": "low_confidence" if ai_result.get("confidence", 0) < 70 else "high_risk",
        ...
        try:
            ...
            notification = Notification(
                ...
                recipients=[
                    ...
                    {
                        "type": "email",
                        "identifier": "risk-team@example.com"
                    }
                ],
            ...
            # Store notification
            notification_id = NotificationRepository.create_notification_sync_obj(notification)
    ...
    @activity.defn
    async def analyze_fraud_network(self, enriched_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if sender_network.get("risk_indicators", {}).get("has_suspicious_patterns"):
                ...
                network_risk_factors.append("Sender involved in suspicious transaction patterns")
            ...
            if recipient_network:
                if recipient_network.get("risk_indicators", {}).get("has_suspicious_patterns"):
                    network_risk_score += 20
                    network_risk_factors.append("Recipient involved in suspicious patterns")
            ...
            if (sender_network.get("risk_indicators", {}).get("has_suspicious_patterns") and
                recipient_network and recipient_network.get("risk_indicators", {}).get("has_suspicious_patterns")):
                network_risk_score += 30
                network_risk_factors.append("Potential money laundering network detected")
            ...
            return {
                ...
                "combined_unique_accounts": (
                    ...
                    (recipient_network.get("unique_accounts_in_networks", 0) if recipient_network else 0)
                ),
